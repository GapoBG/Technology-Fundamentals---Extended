...
namespace _08.Commits
{
    ...
    public class Commits
    {
        public static void Main()
        {
            ...
            var parsedCommitsData = new SortedDictionary<string/*name*/, SortedDictionary<string/*repository*/, List<CommitsData>>>();

            while (URL != "git push")
            {
                var regex = new Regex(@"https:\/\/github.com\/([A-Za-z-0-9]+)\/([A-Za-z-_]+)\/commit\/([a-f0-9]{40}),(.+?)[,](\d+)[,](\d+)");
